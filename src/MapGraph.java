...
public class MapGraph {
	...
	class MapEdge {
		...
		public MapEdge(String[] adjacentNodes, double length, int cctvNum, int shelterNum, int convenienceNum,
				double averageWidth, double averageBrightness, int adultEntNum, int constructionNum) {
		...
		public void update(double length, int cctvNum, int shelterNum, int convenienceNum, double averageWidth,
				double averageBrightness, int adultEntNum, int constructionNum) {
			this.length = length;
			this.cctvNum = cctvNum;
			this.shelterNum = shelterNum;
			this.convenienceNum = convenienceNum;
		...
		public MapVertex getAnotherVertex(MapVertex v) {
			if (this.adjacentNodes[0].equals(v.getId()))
				return findVertexById(adjacentNodes[1]);
			else if (this.adjacentNodes[1].equals(v.getId()))
				return findVertexById(adjacentNodes[0]);
			else
				return null;
		}
	...
	public double getAverageReputation(String id1, String id2) {
		...
		for (int i = 0; i < reput.size(); i++) {
			if ((reput.get(i)[0] == vid1 && reput.get(i)[1] == vid2)
					|| (reput.get(i)[0] == vid2 && reput.get(i)[1] == vid1)) {
				count += 1;
				sum += reput.get(i)[2];
			}
		}
	...
	public void addEdge(String[] adjacentNodes, double length, int cctvNum, int shelterNum, int convenienceNum,
			double averageWidth, double averageBrightness, int adultEntNum, int constructionNum) {
		edges.add(new MapEdge(adjacentNodes, length, cctvNum, shelterNum, convenienceNum, averageWidth,
				averageBrightness, adultEntNum, constructionNum));
	}
	...
	public MapEdge getEdge(MapVertex v, MapVertex u) {
		for (MapEdge e : edges) {
			...
			if (adj1 != null && adj2 != null) {
				if (adj1.equals(v) && adj2.equals(u) || adj1.equals(u) && adj2.equals(v))
					return e;
			}
		}
		return null;
	...
	public void nodeList2Graph(NodeList vertexList, NodeList edgeList) {
		...
		for (int i = 0; i < edgeList.getLength(); i++) {
			...
			MapEdge me = new MapEdge(adjacentNodes, length, cctvNum, shelterNum, convenienceNum, averageWidth,
					averageBrightness, adultEntNum, constructionNum);
			addEdge(me);
